...
In this scenario, we have to consider a Poisson likelihood $L(\mu | x\ H) = \prod_{i=1}^{n} \frac{\mu^{x_i} e^{-\mu}}{x_i!}$.

1.1. Assuming a positive uniform prior distribution for $\mu \in [1, 20]$:

  -   Recalling that the Gamma distribution is defined as $\text{Gamma}(x | \alpha, \beta) = \frac{x^{\alpha - 1} e^{-x\beta}\beta^{\alpha} }{\Gamma(\alpha)}$, the posterior takes the form:  $P(\mu | x\ H) = \frac{L(\mu | x)}{\int_0^{\infty} L(\lambda | x) d\lambda} = \text{Gamma}(\mu | \sum_i{x_i} +1, N)$, where N is the number of samples. So we can estimate: 
    
  * $E(X) = \frac \alpha \beta =  \frac{\sum_i{x_i} +1}{N} = 3$
  * $\text{SD}(X) = \frac{\sqrt{\alpha}}{\beta} = 0.7$

1.2. Assuming instead a Gamma prior with the parameters  $\alpha^\prime=9$ and $\beta^\prime=3$:

  - Since the Gamma is the conjugate prior of a Poisson likelihood, the posterior takes the form $P(\mu | x\ H) = \text{Gamma}(\alpha^\prime + \sum_i{x_i}, \beta^\prime + N)$. Thus we can estimate:
  
  * $E(X) = \frac \alpha \beta = \frac {9+17}{3+6} = 2.89$
  * $\text{SD}(X) = \frac{\sqrt{\alpha}}{\beta} = 0.57$

1.3. Using the following code we can estimate the previous points numerically and compute the 95% credibility intervals:
...
curve(uniform_posterior, from = 0, to = 8, xlab = expression(mu), ylab = 'Posterior', col=color_vector[1], main='Activity using a uniform prior', xlim = c(0,8), lwd=2, n = 500) 
...
The likelihood of this problem is binomial: $L(p | r, n) = \binom{n}{r} p^r (1-p)^{n-r}$.
Let's also recall that a Beta distribution takes the form $\text{Beta} (x | \alpha, \beta) = \frac{x^{\alpha - 1} (1 - x)^{\beta - 1}}{B(\alpha, \beta)}$.

2.1. Analytically
  - Under the hypothesis of a  uniform prior U(0, 1), that can be modeled by a $\text{Beta} (p | 1, 1)$, the posterior takes the form $P(p | x \ H)  = \text{Beta}(p| r + 1, 1 + n - r) = \text{Beta}(p| 313, 189)$.
    
  * $E[p] = \frac {\alpha}{\alpha+\beta} = 0.62$
  * $SD[p] = \sqrt{\frac {\alpha\beta}{\alpha+\beta+1}}{(\alpha+\beta)}^{-1} = 0.02$ 
      
  - Under the hypothesis of a  Jeffrey’s prior Beta(1/2, 1/2), instead, the posterior takes the form $P(p | x \ H)  = \text{Beta}(p| r + \frac 1 2, \frac 1 2 + n - r)$, leading approximately at the same results in terms of mean and standard deviation.
    
2.2.  Using the following code we can estimated the previous points numerically and produce the plots:
...
2.3. In this case, the posterior is $P(p | x \ H)  = \text{Beta}(p| r + 1, 1 + n - r) = \text{Beta}(p| 11, 1) $.
  * $E[p] = \frac {\alpha}{\alpha+\beta} = 0.92$
  * $SD[p] = \sqrt{\frac {\alpha\beta}{\alpha+\beta+1}}{(\alpha+\beta)}^{-1} = 0.08$
...
2.4. Using as a prior the posterior of Researcher B we obtain: $P(p | x \ H)  = \text{Beta}(p| r + 11, 1 + n - r) = \text{Beta}(p| 323, 189)$;
  * $E[p] = \frac {\alpha}{\alpha+\beta} = 0.63$
  * $SD[p] = \sqrt{\frac {\alpha\beta}{\alpha+\beta+1}}{(\alpha+\beta)}^{-1} = 0.02$ 
...
```{r 2.5}
credibility_95_interval <- sapply(c(0.025, 0.975), function(P) inverse_cumulative(A.uniform_B_posterior, p = P, lower = 0, upper = 1))

x_plot <- seq(from=credibility_95_interval[1], to=credibility_95_interval[2], length.out=100)
y_plot <- c(0, A.uniform_B_posterior(x_plot), 0)
x_plot <- c(credibility_95_interval[1], x_plot, credibility_95_interval[2])

curve(A.uniform_B_posterior, from = 0, to = 1, xlab = expression(Efficiency), ylab = 'Posterior', col=color_vector[1], main="Researcher A with Researcher's B prior", xlim=c(0.5, 0.75), lwd=2, n = 500) 

polygon(x_plot, y_plot, col = adjustcolor(color_vector[4], alpha.f = 0.25),border = NA)

abline(v = A.mean_uniform_B, col = color_vector[7], lwd=2, lty='dashed')
abline(v = A.mean_uniform_B - A.std_uniform_B, col = color_vector[6], lwd=2, lty='dashed')
abline(v = A.mean_uniform_B + A.std_uniform_B, col = color_vector[6], lwd=2, lty='dashed')
...
uniform_interval_lenght_history <- lapply(uniform_posterior_history(), calculate_95_interval) |>
  (\(my_list) sapply(my_list, function(x) x[2] - x[1]))()
beta_interval_lenght_history <- lapply(beta_posterior_history(), calculate_95_interval) |>
  (\(my_list) sapply(my_list, function(x) x[2] - x[1]))()
...
lines(N, uniform_interval_lenght_history, col = color_vector[1], lwd = 2, lty=2)
lines(N, beta_interval_lenght_history, col = color_vector[6], lwd = 2, lty=2)

legend("topright", legend = c("Max of Uniform Prior", "Max of Beta(10, 10) Prior", "Uniform CI", "Beta CI"), col = c(color_vector[1], color_vector[6], color_vector[1], color_vector[6]), lty = c(1,1,2,2), lwd=2)
...
writeLines(sprintf(
  ...
  - Assuming a beta(10,10) prior, the most probable value is %.3f
  - The 95%% credibility interval is [ %.2f , %.2f ]
  ', tail(uniform_max_history, 1), tail(uniform_interval_history, 1)[[1]][1], tail(uniform_interval_history, 1)[[1]][2], tail(beta_max_history, 1), tail(beta_interval_history, 1)[[1]][1], tail(beta_interval_history, 1)[[1]][2]
  )
)
...
The underlying distribution in this case is a Multinomial with 4 possible outcomes: $P(\mathbf{x} | \pi_1, \pi_2, ..., \pi_k) = \binom{n}{x_1, x_2, ..., x_k} \prod_{i=1}^{k} \pi_i^{x_i}$ with $k=4$
The conjugate prior for a multinomial likelihood is a Dirichlet function: $\text{Dirichlet}(x| \alpha) = \frac{1}{B(\alpha)} \prod_{i=1}^k x_i^{\alpha_i - 1}$.
Given a prior $\pi (p|H) =  \text{Dirichlet}( p | \alpha^\prime)$, the posterior becomes $P(p|x\ H) = \text{Dirichlet}( p | \alpha^\prime + \sum_i{x_i})$.

Furthermore, let's observe that when $\vec{\alpha}= \vec{1}$ the Dirichlet function reduces to a uniform one.
...
# Prepare the colors
uniform_color <- color_vector[5]
poll_color    <- color_vector[4]  
...
ggplot(data_melt, aes(x = Label, y = value, fill = Prior)) +
  ...
  labs(title = "Posterior Distributions",
       ...
       x = "Labels") +
  theme_minimal() +
  scale_fill_manual(values = c("Uniform" = uniform_color, "Poll-based" = poll_color))
...
ggplot(df_means, aes(x = Label, y = Mean, fill = Prior)) +
  ...
  geom_errorbar(aes(ymin = Lower, ymax = Upper), position = position_dodge(width = 0.7), width = 0.2, color=color_vector[6]) +
  labs(title = "Posterior Means with 68% Credibility Intervals",
       y = "Mean",
       x = "Labels") +
  theme_minimal() +
  scale_fill_manual(values = c("Uniform Prior" = uniform_color, "Poll-based Prior" = poll_color))
