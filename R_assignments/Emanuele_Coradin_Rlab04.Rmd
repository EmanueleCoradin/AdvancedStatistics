...
curve(beta_posterior, from = 0, to = 1, xlab = TeX('$p_{fail}$'), ylab = 'Posterior', col=color_vector[1], main='P', lwd=2, n = 500, xlim=c(0,0.3)) 
...
legend("topright", legend = c("Beta Posterior", "Mean", "±σ"), col = c(color_vector[1], color_vector[7], color_vector[6]), lty = c('solid', 'longdash', 'dashed'))
...
```
    (e) In the frequentist approach, we can use a binomial test. Looking at the results, with this approach it is not possible to reject the hypotesis that the new method is worse than the previous one at a 5% level of significance, since the 95% confidence interval includes p = 15%.
...
curve(posterior_values, from = 0, to = 8, xlab = expression(mu), ylab = 'Posterior', col=color_vector[1], main='P', lwd=2, n = 500, xlim=c(0,8)) 
...
sapply(2:N_box, function(ibox) lines(posterior_matrix[ibox, ], type = 'o', pch = 20, col = colors[ibox], lty = line_types[ibox], lwd=2))
